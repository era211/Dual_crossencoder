...
def structure_pair_dual(mention_1,
                   mention_2,
                   doc_dict,
...
def structure_dataset_for_eval(data_set,
    ...
    for mention_1, mention_2 in tqdm(pairs[:2000]):  # 从提及对数据列表中分别读取每一对mention
...
def structure_data_for_train(df):  # 构建训练数据对
    ...
    all_syn_adj = []
    for ID in tqdm(all_data_index[:5000], desc='structure_train'):
        # get 'label'
...
def is_cluster_merge(cluster_1, cluster_2, mentions, model, doc_dict):
    ...
    if len(cluster_1) > sample_size:
        c_1 = set(random.sample(list(cluster_1), sample_size))
    else:
        c_1 = cluster_1
    if len(cluster_2) > sample_size:
        c_2 = set(random.sample(list(cluster_2), sample_size))
    else:
        c_2 = cluster_2
...
def transitive_closure_merge(edges, mentions, model, doc_dict, graph,
    ...
    clusters = {}  # 聚类字典
    inv_clusters = {}
    mentions = {mention.mention_id: mention for mention in mentions}  # 将 mentions 转换为字典，键是 mention_id，值是对应的提及对象
    for edge in tqdm(edges):  # prob大于0.5的两个提及的索引，标签和概率  [()]
        cluster_key = find_cluster_key(edge[0], clusters)  # 查找提及的聚类键，第一个提及对应的聚类键
        alt_key = find_cluster_key(edge[1], clusters)  # 第二个提及对应的聚类键
        if cluster_key == None and alt_key == None:  # 当前提及对中的两个提及都不属于任何现有的聚类
            cluster_key = edge[0]  # 将第一个提及（edge[0]）作为新的聚类键，并在 clusters 字典中为这个键创建一个新的空集合
            clusters[cluster_key] = set()
        elif cluster_key == None and alt_key != None:  # 一个提及属于已有聚类，而另一个不属于任何聚类：将不属于聚类的提及添加到已有聚类
            cluster_key = alt_key
            alt_key = None
        elif cluster_key == alt_key:  # 两个提及已经在同一个聚类中：不进行任何操作，因为它们已经在同一个聚类中
            alt_key = None
