...
if use_cuda:
    ...
    def from_numpy(ndarray):
        # if float(sys.version[:3]) <= 3.6: # 我这里的python版本是3.10
        #     # return eval('torch.from_numpy(ndarray).pin_memory().cuda(device=device, async=True)')
        #     return eval('torch.from_numpy(ndarray).pin_memory().cuda(device=device)')
        # else:
        #     return torch.from_numpy(ndarray).pin_memory().cuda(device=device, non_blocking=True)
        return torch.from_numpy(ndarray).pin_memory().cuda(device=device, non_blocking=True)
else:
...
from src_joint import hpsg_decoder
from src_joint import const_decoder
from src_joint import makehp
from src_joint import utils

from src_joint import trees
...
class BatchIndices:
    ...
    def __init__(self, batch_idxs_np):
        self.batch_idxs_np = batch_idxs_np  # list
        self.batch_idxs_torch = from_numpy(batch_idxs_np)  # 将list转换为tensor

        self.batch_size = int(1 + np.max(batch_idxs_np))  # 1

        batch_idxs_np_extra = np.concatenate([[-1], batch_idxs_np, [-1]])
        self.boundaries_np = np.nonzero(batch_idxs_np_extra[1:] != batch_idxs_np_extra[:-1])[0]
        self.seq_lens_np = self.boundaries_np[1:] - self.boundaries_np[:-1]  # 得到序列的长度
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, hparams, n_head, d_model, d_k, d_v, residual_dropout=0.1, attention_dropout=0.1, d_positional=None):
        ...
        if self.partitioned:
            ...
            self.w_qs1 = nn.Parameter(torch_t.FloatTensor(n_head, self.d_content, d_k // 2).to(device))
            self.w_ks1 = nn.Parameter(torch_t.FloatTensor(n_head, self.d_content, d_k // 2).to(device))
            self.w_vs1 = nn.Parameter(torch_t.FloatTensor(n_head, self.d_content, d_v // 2).to(device))

            self.w_qs2 = nn.Parameter(torch_t.FloatTensor(n_head, self.d_positional, d_k // 2).to(device))
            self.w_ks2 = nn.Parameter(torch_t.FloatTensor(n_head, self.d_positional, d_k // 2).to(device))
            self.w_vs2 = nn.Parameter(torch_t.FloatTensor(n_head, self.d_positional, d_v // 2).to(device))
            ...
            self.w_qs = nn.Parameter(torch_t.FloatTensor(n_head, d_model, d_k).to(device))
            self.w_ks = nn.Parameter(torch_t.FloatTensor(n_head, d_model, d_k).to(device))
            self.w_vs = nn.Parameter(torch_t.FloatTensor(n_head, d_model, d_v).to(device))
...
class MultiLevelEmbedding(nn.Module):
    def __init__(self,
        ...
        self.position_table = nn.Parameter(torch_t.FloatTensor(max_len, self.d_positional).to(device))
...
def get_xlnet(xlnet_model, xlnet_do_lower_case):
    ...
    from transformers import (WEIGHTS_NAME, XLNetModel,
                                      ...
                                      XLNetForSequenceClassification,
                                      XLNetTokenizer)
    tokenizer = XLNetTokenizer.from_pretrained('xlnet-large-cased', do_lower_case=xlnet_do_lower_case)
    xlnet = XLNetModel.from_pretrained('xlnet-large-cased')
...
class BiAAttention(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.dep_weight = nn.Parameter(torch_t.FloatTensor(hparams.d_biaffine + 1, hparams.d_biaffine + 1).to(device))
        nn.init.xavier_uniform_(self.dep_weight)

    def forward(self, input_d, input_e, input_s = None):

        score = torch.matmul(torch.cat(
            [input_d, torch_t.FloatTensor(input_d.size(0), 1).fill_(1).requires_grad_(False).to(device)],
            dim=1), self.dep_weight)
        score1 = torch.matmul(score, torch.transpose(torch.cat(
            [input_e, torch_t.FloatTensor(input_e.size(0), 1).fill_(1).requires_grad_(False).to(device)],
            dim=1), 0, 1))
...
class LabelAttention(nn.Module):
    ...
    def __init__(self, hparams, d_model, d_k, d_v, d_l, d_proj, use_resdrop=True, q_as_matrix=False, residual_dropout=0.1, attention_dropout=0.1, d_positional=None):
        ...
        if self.partitioned:
            ...
            if self.q_as_matrix:
                self.w_qs1 = nn.Parameter(torch_t.FloatTensor(self.d_l, self.d_content, d_k // 2).to(device), requires_grad=True)
            else:
                self.w_qs1 = nn.Parameter(torch_t.FloatTensor(self.d_l, d_k // 2).to(device), requires_grad=True)
            self.w_ks1 = nn.Parameter(torch_t.FloatTensor(self.d_l, self.d_content, d_k // 2).to(device), requires_grad=True)
            self.w_vs1 = nn.Parameter(torch_t.FloatTensor(self.d_l, self.d_content, d_v // 2).to(device), requires_grad=True)

            if self.q_as_matrix:
                self.w_qs2 = nn.Parameter(torch_t.FloatTensor(self.d_l, self.d_positional, d_k // 2).to(device), requires_grad=True)
            else:
                self.w_qs2 = nn.Parameter(torch_t.FloatTensor(self.d_l, d_k // 2).to(device), requires_grad=True)
            self.w_ks2 = nn.Parameter(torch_t.FloatTensor(self.d_l, self.d_positional, d_k // 2).to(device), requires_grad=True)
            self.w_vs2 = nn.Parameter(torch_t.FloatTensor(self.d_l, self.d_positional, d_v // 2).to(device), requires_grad=True)
            ...
            if self.q_as_matrix:
                self.w_qs = nn.Parameter(torch_t.FloatTensor(self.d_l, d_model, d_k).to(device), requires_grad=True)
            else:
                self.w_qs = nn.Parameter(torch_t.FloatTensor(self.d_l, d_k).to(device), requires_grad=True)
            self.w_ks = nn.Parameter(torch_t.FloatTensor(self.d_l, d_model, d_k).to(device), requires_grad=True)
            self.w_vs = nn.Parameter(torch_t.FloatTensor(self.d_l, d_model, d_v).to(device), requires_grad=True)
...
class ChartParser(nn.Module):
    def __init__(
        ...
        if hparams.use_xlnet:
            ...
            self.xlnet_tokenizer = XLNetTokenizer.from_pretrained('/home/yaolong/Rationale4CDECR-main/src/LAL_Parser/src_joint/xlnet/xlnet-large-cased', do_lower_case=hparams.xlnet_do_lower_case)
            self.xlnet = XLNetModel.from_pretrained('/home/yaolong/Rationale4CDECR-main/src/LAL_Parser/src_joint/xlnet/xlnet-large-cased')
            if hparams.bert_transliterate:
                from src.LAL_Parser.src_joint.transliterate import TRANSLITERATIONS
                self.bert_transliterate = TRANSLITERATIONS[hparams.bert_transliterate]
            else:
                self.bert_transliterate = None

            d_xlnet_annotations = self.xlnet.d_model  # 1024
        ...
        if hparams.use_bert or hparams.use_bert_only:
            ...
            # / home / yaolong / PT_MODELS / PT_MODELS / bert - large - cased
    ...
    @classmethod
    def from_spec(cls, spec, model):  # LAL模型中的超参数以及模型参数字典
        spec = spec.copy()
    ...
    def parse_batch(self, sentences, golds=None):
        ...
        if self.xlnet is not None:
            ...
            all_input_ids = np.zeros((len(sentences), self.xlnet_max_len), dtype=int)   # (1,768)的数组
